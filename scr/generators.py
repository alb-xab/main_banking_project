...
def filter_by_currency(transactions: list, currency:str='USD'):
    ...
    for transaction in transactions:
        if transaction is dict and 'currency' in transaction:
            if transaction['currency']['code'] == currency:
                yield transaction
        return 'Ошибка вводных данных. Повторите попытку'
...
def card_number_generator(start:int=1, stop:int=9999999999999999):
    """ Функция для случайной генерации номера карты в 16-символьном формате с заданными начальными и конечными рамками"""
    final_number_card = []
